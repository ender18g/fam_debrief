...
def create_report(dir,csv_file,start_r=None,end_r=None):
    ...
    if start_r and end_r:
        responseArray=responseArray[int(start_r-2):int(end_r)-1]

    file_name = dir+'pdf/'+csv_file.split('.')[0]
    ...
    plots={n:createPlot(n) for n in [1,3,5,7]}  # These are the questions that require plots
    remove_list=[]
    ...
    for num, (n,plot) in enumerate(plots.items()):
        created_Plot = plot
        if created_Plot != 0:
...
if __name__ == '__main__':
    create_report('uploads/','responses.csv',166,195)